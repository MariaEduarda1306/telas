...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndDisplayProgress(userId, token) {
        try {
            ...
            document.querySelector('.progress-label').textContent = `${totalCompleted} / ${totalRequired} Horas`;

        } catch (error) {
            console.error("Erro ao carregar progresso:", error);
            document.querySelector('.progress-label').textContent = "Erro ao carregar dados.";
        }
    }
    ...
    if (passwordModal) {
      ...
      passwordForm.addEventListener('submit', async (e) => {
        ...
        // 1. O backend (UpdateUsuarioRequest.php) espera apenas os campos
        //    'password' e 'password_confirmation'.
        const data = {
            password: newPassword,
            password_confirmation: confirmPassword
            // O campo 'current_password' é coletado no HTML, mas o backend
            // atual não o valida. Veja a "Nota Importante" abaixo.
        };

        try {
            // 2. A rota correta é PUT /api/usuarios/{id}
            const response = await fetch(`${API_BASE_URL}/api/usuarios/${userData.id}`, {
                method: 'PUT', // 3. O método correto é PUT
            // ##########################################
                headers: {
                    'Authorization': `Bearer ${authToken}`,
