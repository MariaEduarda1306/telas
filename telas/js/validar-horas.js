...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndRenderStudents() {
        ...
        try {
            const response = await fetch(`http://localhost:8000/api/certificados?status=ENTREGUE`, {
    ...
    async function showDetailView(studentId, studentName) {
        ...
        try {
            const response = await fetch(`http://localhost:8000/api/usuarios/${studentId}/historico`, {
                headers: { 'Authorization': `Bearer ${authToken}`, 'Accept': 'application/json' }
            });
    ...
    async function handleEvaluation(certificateId, newStatus, studentId, studentName) {
        ...
        try {
            const response = await fetch(`http://localhost:8000/api/certificados/${certificateId}/avaliar`, {
                method: 'PATCH',
                headers: {
