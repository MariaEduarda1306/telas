...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndRenderStudents() {
        ...
        try {
            ...
            // 2. Agrupa por aluno (Lógica original mantida)
            const studentsMap = {};
            ...
            // 3. APLICAÇÃO DOS FILTROS (Aqui estava o ponto crítico)
            // Lemos os valores dos inputs aqui dentro para filtrar o array já carregado
            ...
            const filterFase = document.getElementById('fase').value;

            if (filterNome) {
                studentsArray = studentsArray.filter(s => s.nome && s.nome.toLowerCase().includes(filterNome));
            }
            if (filterMatricula) {
                studentsArray = studentsArray.filter(s => String(s.matricula).includes(filterMatricula));
            }
            if (filterFase) {
                studentsArray = studentsArray.filter(s => String(s.fase) === filterFase);
            }
            ...
            studentsArray.forEach(aluno => {
                ...
                row.innerHTML = `
                    <td data-label="Notificação"><i class="fas fa-bell notification-bell" style="color:var(--status-ressalva)" title="Nova atividade!"></i></td>
                    <td data-label="Nome do Aluno"><strong>${aluno.nome}</strong></td>
                    <td data-label="Matrícula">${aluno.matricula}</td>
                    <td data-label="Solicitações Pendentes"><span class="status status-ressalva">${aluno.pending_count}</span></td>
                `;
    ...
    // =======================================================
    // ETAPA 2: EXIBIR DETALHES (Mantido igual)
    // =======================================================
    ...
    // =======================================================
    // ETAPA 3: ENVIAR AVALIAÇÃO (Mantido igual)
    // =======================================================
    ...
    // O botão "Filtrar" agora chama fetchAndRenderStudents() novamente.
    // Como a função lê os inputs dentro dela, a filtragem acontecerá.
    if (filterBtn) {
        filterBtn.addEventListener('click', fetchAndRenderStudents);
    }
    ...
    if (clearFiltersBtn) {
        clearFiltersBtn.addEventListener('click', () => {
            ...
            if (faseSelect) {
                faseSelect.value = '';
                // Reseta visualmente o custom select, se houver
